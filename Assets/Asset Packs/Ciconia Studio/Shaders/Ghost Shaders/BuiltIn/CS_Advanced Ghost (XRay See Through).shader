Shader "Ciconia Studio/CS_Ghost/CS_Advanced Ghost (XRay See Through)"
{
	Properties
	{
	...
	SubShader
	{
		...
		void outlineSurf( Input i, inout SurfaceOutput o )
		{
			...
			float OpacityFresnelSub600 = temp_output_7_0_g1;
			o.Emission = saturate( ( fresnelNode536 * _XRayColor ) ).rgb;
			o.Alpha = ( ( fresnelNode536 * _XRayColor.a ) * OpacityFresnelSub600 );
		...
		uniform float _XRayFresnelIntensity;
		uniform float _XRayFresnelPower;
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float3 NormalmapXYZ572 = UnpackScaleNormal( tex2D( _BumpMap, uv_BumpMap ), _BumpScale );
			#if defined(LIGHTMAP_ON) && UNITY_VERSION < 560 //aseld
			float3 ase_worldlightDir = 0;
			#else //aseld
			float3 ase_worldlightDir = normalize( UnityWorldSpaceLightDir( ase_worldPos ) );
			#endif //aseld
			float dotResult577 = dot( (WorldNormalVector( i , NormalmapXYZ572 )) , ase_worldlightDir );
			...
			#endif //aselc
			UnityGI gi585 = gi;
			float3 diffNorm585 = WorldNormalVector( i , NormalmapXYZ572 );
			gi585 = UnityGI_Base( data, 1, diffNorm585 );
			float3 indirectDiffuse585 = gi585.indirect.diffuse + diffNorm585 * 0.0001;
			float3 indirectNormal594 = WorldNormalVector( i , NormalmapXYZ572 );
			Unity_GlossyEnvironmentData g594 = UnityGlossyEnvironmentSetup( _Glossiness, data.worldViewDir, indirectNormal594, float3(0,0,0));
			float3 indirectSpecular594 = UnityGI_IndirectSpecular( data, 1.0, indirectNormal594, g594 );
			float3 ase_worldTangent = WorldNormalVector( i, float3( 1, 0, 0 ) );
			float3 ase_worldBitangent = WorldNormalVector( i, float3( 0, 1, 0 ) );
			float3x3 ase_tangentToWorldFast = float3x3(ase_worldTangent.x,ase_worldBitangent.x,ase_worldNormal.x,ase_worldTangent.y,ase_worldBitangent.y,ase_worldNormal.y,ase_worldTangent.z,ase_worldBitangent.z,ase_worldNormal.z);
			float fresnelNdotV584 = dot( mul(ase_tangentToWorldFast,NormalmapXYZ572), ase_worldViewDir );
			...
			#endif
			float4 lerpResult595 = lerp( ( ( _Color * tex2D( _MainTex, uv_MainTex ) ) * float4( ( ( max( dotResult577 , 0.0 ) * ( ase_lightAtten * ase_lightColor.rgb ) ) + indirectDiffuse585 ) , 0.0 ) ) , float4( indirectSpecular594 , 0.0 ) , staticSwitch592);
			float4 CustomLight596 = lerpResult595;
			float OpacityFresnelSub600 = temp_output_7_0_g1;
			c.rgb = ( CustomLight596 * OpacityFresnelSub600 ).rgb;
			c.a = Opacity250;
			return c;
		}
		...
		void surf( Input i , inout SurfaceOutputCustomLightingCustom o )
		{
			...
			float mulTime562 = _Time.y * _AmplitudeSpeed;
