		...
		for (int i = 0; i < 3; i++)
			{
				...
				final += pow(saturate((pow(sampledCustomNoise.x, NoisePow) * NoiseStrength * 2.75)), 1.25) * saturate(1.0-(i/20.0)) * linearMask;
				final2 += pow(sampledCustomNoise.y, VolumeNoise2Exp * 0.75) * VolumeNoise2Multiply * 3.0 * linearMask;
